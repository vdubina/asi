@extends('layouts.admin')
@section('content')

<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.taxonomyMediaDelivery.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.taxonomy-media-deliveries.update", [$taxonomyMediaDelivery->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.taxonomyMediaDelivery.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $taxonomyMediaDelivery->name) }}" required>
                @if($errors->has('name'))
                    <span class="text-danger">{{ $errors->first('name') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.taxonomyMediaDelivery.fields.name_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="description">{{ trans('cruds.taxonomyMediaDelivery.fields.description') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $taxonomyMediaDelivery->description) !!}</textarea>
                @if($errors->has('description'))
                    <span class="text-danger">{{ $errors->first('description') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.taxonomyMediaDelivery.fields.description_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="field_offsiteid">{{ trans('cruds.taxonomyMediaDelivery.fields.field_offsiteid') }}</label>
                <input class="form-control {{ $errors->has('field_offsiteid') ? 'is-invalid' : '' }}" type="number" name="field_offsiteid" id="field_offsiteid" value="{{ old('field_offsiteid', $taxonomyMediaDelivery->field_offsiteid) }}" step="1" required>
                @if($errors->has('field_offsiteid'))
                    <span class="text-danger">{{ $errors->first('field_offsiteid') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.taxonomyMediaDelivery.fields.field_offsiteid_helper') }}</span>
            </div>
            <div class="form-group">
                <div class="form-check {{ $errors->has('field_isshipped') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="field_isshipped" value="0">
                    <input class="form-check-input" type="checkbox" name="field_isshipped" id="field_isshipped" value="1" {{ $taxonomyMediaDelivery->field_isshipped || old('field_isshipped', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="field_isshipped">{{ trans('cruds.taxonomyMediaDelivery.fields.field_isshipped') }}</label>
                </div>
                @if($errors->has('field_isshipped'))
                    <span class="text-danger">{{ $errors->first('field_isshipped') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.taxonomyMediaDelivery.fields.field_isshipped_helper') }}</span>
            </div>
            <div class="form-group">
                @include('partials.buttons.save')
                @include('partials.buttons.back', ['url'=>route('admin.taxonomy-media-deliveries.index')])
            </div>
        </form>
    </div>
</div>



@endsection

@section('scripts')
    @include('partials.scripts.simpleUpload',['route'=>'taxonomy-media-deliveries', 'id'=>$taxonomyMediaDelivery->id])
@endsection
