...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.taxonomyCourse.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.taxonomy-courses.update", [$taxonomyCourse->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.taxonomyCourse.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $taxonomyCourse->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $taxonomyCourse->description) !!}</textarea>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('field_available') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="field_available" id="field_available" value="1" {{ $taxonomyCourse->field_available || old('field_available', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('field_placeholder') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="field_placeholder" id="field_placeholder" value="1" {{ $taxonomyCourse->field_placeholder || old('field_placeholder', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('field_dental_report_text') ? 'is-invalid' : '' }}" type="text" name="field_dental_report_text" id="field_dental_report_text" value="{{ old('field_dental_report_text', $taxonomyCourse->field_dental_report_text) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('field_course_type') ? 'is-invalid' : '' }}" name="field_course_type_id" id="field_course_type_id">
                    @foreach($field_course_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('field_course_type_id') ? old('field_course_type_id') : $taxonomyCourse->field_course_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('field_additional_information') ? 'is-invalid' : '' }}" name="field_additional_information" id="field_additional_information">{!! old('field_additional_information', $taxonomyCourse->field_additional_information) !!}</textarea>
            ...
            <div class="form-group">
                ...
                @include('partials.buttons.back', ['url'=>route('admin.taxonomy-courses.index')])
            </div>
        </form>
    </div>
</div>
...
@section('scripts')
    @include('partials.scripts.simpleUpload',['route'=>'taxonomy-courses', 'id'=>$taxonomyCourse->id])
@endsection
