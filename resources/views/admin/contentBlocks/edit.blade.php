...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.content-blocks.update", [$contentBlockType->id, $contentBlock->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.contentBlock.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $contentBlock->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('content') ? 'is-invalid' : '' }}" name="content" id="content">{!! old('content', $contentBlock->content) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type_id" id="type_id" required>
                    @foreach($contentBlockTypes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('type_id') ? old('type_id') : $contentBlock->type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('show_on_pages') ? 'is-invalid' : '' }}" name="show_on_pages[]" id="show_on_pages" multiple>
                    @foreach($show_on_pages as $id => $show_on_page)
                        <option value="{{ $id }}" {{ (in_array($id, old('show_on_pages', [])) || $contentBlock->show_on_pages->contains($id)) ? 'selected' : '' }}>{{ $show_on_page }}</option>
                    @endforeach
                </select>
...
<script>
    ...
    url: '{{ route('admin.content-blocks.storeMedia',$contentBlockType->id) }}',
    maxFilesize: 2, // MB
    ...
    init: function () {
@if(isset($contentBlock) && $contentBlock->images)
      var file = {!! json_encode($contentBlock->images) !!}
