...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.content-blocks.store",$contentBlockType->id) }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type_id" id="type_id" required>
                    @foreach($contentBlockTypes as $id => $entry)
                        <option value="{{ $id }}" {{ $contentBlockType->id == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.content-blocks.storeCKEditorImages',$contentBlockType->id) }}', true);
...
<script>
    ...
    url: '{{ route('admin.content-blocks.storeMedia') }}',
    maxFilesize: 2, // MB
