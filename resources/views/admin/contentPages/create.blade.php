        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.content-pages.store") }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('slider') ? 'is-invalid' : '' }}"
                            name="slider_id" id="slider_id">
                        @foreach($sliders as $id => $entry)
                            <option
                                value="{{ $id }}" {{ old('slider_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('field_is_subsite_content') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="field_is_subsite_content"
                               id="field_is_subsite_content"
                               value="1" {{ old('field_is_subsite_content', 0) == 1 ? 'checked' : '' }}>
                        <label class="form-check-label"
                               for="field_is_subsite_content">{{ trans('cruds.contentPage.fields.field_is_subsite_content') }}</label>
                    </div>
                    @if($errors->has('field_is_subsite_content'))
                        <span class="text-danger">{{ $errors->first('field_is_subsite_content') }}</span>
                    @endif
                    <span
                        class="help-block">{{ trans('cruds.contentPage.fields.field_is_subsite_content_helper') }}</span>
                </div>
                <div class="form-group">
...
@section('scripts')
    @include('partials.scripts.simpleUpload',['route'=>'content-pages',])
    @include('partials.scripts.imagesDropzone',['route'=>'content-pages','key'=>'featured_image',])
@endsection
