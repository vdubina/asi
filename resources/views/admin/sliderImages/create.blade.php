          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.slider-images.storeCKEditorImages') }}', true);
                ...
                data.append('crud_id', '{{ $sliderImage->id ?? 0 }}');
                xhr.send(data);
              });
            })
        }
...
<script>
    Dropzone.options.fieldSlideImageDropzone = {
    url: '{{ route('admin.slider-images.storeMedia') }}',
    maxFilesize: 2, // MB
    ...
    success: function (file, response) {
      $('form').find('input[name="field_slide_image"]').remove()
      $('form').append('<input type="hidden" name="field_slide_image" value="' + response.name + '">')
    },
    ...
    init: function () {
@if(isset($sliderImage) && $sliderImage->field_slide_image)
      var file = {!! json_encode($sliderImage->field_slide_image) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview)
      file.previewElement.classList.add('dz-complete')
      $('form').append('<input type="hidden" name="field_slide_image" value="' + file.file_name + '">')
      this.options.maxFiles = this.options.maxFiles - 1
@endif
    },
