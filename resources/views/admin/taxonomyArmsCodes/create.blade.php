...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.taxonomyArmsCode.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.taxonomy-arms-codes.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('field_arms_category') ? 'is-invalid' : '' }}" name="field_arms_category_id" id="field_arms_category_id" required>
                    @foreach($field_arms_categories as $id => $entry)
                        <option value="{{ $id }}" {{ old('field_arms_category_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.taxonomy-arms-codes.storeCKEditorImages') }}', true);
                ...
                data.append('crud_id', '{{ $taxonomyArmsCode->id ?? 0 }}');
                xhr.send(data);
              });
            })
        }
